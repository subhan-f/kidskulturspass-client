...
import { ArrowClockwise, Calendar3, ChevronDown, ChevronUp } from 'react-bootstrap-icons';
...
function UnassignedEventsDashboard({ setAuth }) {
  ...
  const [loadingMessage, setLoadingMessage] = useState("Veranstaltungen werden geladen...");
  ...
  const fetchEvents = async (options = {}) => {
    const { showLoading = true, isPolling = false, forceRefresh = false } = options;
    ...
    try {
      ...
      if (response.data.status === 'loading') {
        ...
        if (!polling) {
          ...
          if (showLoading) {
            setLoading(true);
            setLoadingMessage("Daten werden vom Kalender-Service geladen. Dies kann bis zu einer Minute dauern...");
          }
  ...
  const pollForEvents = async () => {
    ...
    try {
      ...
      if (!response.data.status || response.data.status !== 'loading') {
        ...
        setPolling(false);
        setLoading(false);
      } else if (pollingAttempts < 24) { // Increased polling attempts for 10 minute timeout (24 attempts = 2 minutes)
        ...
        setPolling(false);
        setLoading(false);
        setWarning("Timeout beim Laden der Daten. Bitte aktualisieren Sie die Seite oder versuchen Sie es später erneut.");
  ...
  const getRoleCountsByCalendar = useMemo(() => {
    return events.reduce((acc, event) => {
      ...
      roles.forEach(role => {
        ...
        acc[event.calendar][trimmedRole] = (acc[event.calendar][trimmedRole] || 0) + 1;
      });
  ...
  const calendarHasMatch = useCallback((calendar) => {
    return filteredEventsByCalendar[calendar] && filteredEventsByCalendar[calendar].length > 0;
  }, [filteredEventsByCalendar]);
    ...
    return (
      ...
      >
        <DashboardLoader 
          ...
          progressMessage={polling ? `Anfrage läuft... ${pollingAttempts > 0 ? `(Versuch ${pollingAttempts})` : ''}` : null}
        />
      </DashboardLayout>
    );
  }
    ...
    >
      <div className="unassigned-events-dashboard">
        ...
        <div className="transparent-header-container">
          <h1 className="dashboard-main-title">Unzugewiesene Veranstaltungen</h1>
        ...
        {warning && <Alert variant="warning" className="dashboard-alert">{warning}</Alert>}
          ...
          {totalFilteredEvents === 0 ? (
            <div className="empty-state">
              ...
              <p className="empty-state-message">
                {searchTerm ? 'Keine Ergebnisse gefunden.' : 'Keine unzugewiesenen Veranstaltungen gefunden.'}
              </p>
            </div>
          ) : (
            calendars.map((calendar) => (
              <div 
                key={calendar} 
                className={`event-calendar-card ${!calendarHasMatch(calendar) && searchTerm ? 'filtered-out' : ''}`}
              >
                <div 
                ...
                >
                  <div className="header-content">
                    <div className="title-with-icon">
                      ...
                      {/* Add role badges in header - similar to ArtistsDashboard */}
                    ...
                    <span className="events-count">
                      <span className="count-number">{filteredEventsByCalendar[calendar].length}</span>
                      <span className="count-label">{filteredEventsByCalendar[calendar].length === 1 ? ' Veranstaltung' : ' Veranstaltungen'}</span>
                    </span>
                  </div>
                </div>
                            ...
                            <tr key={index} className="event-row">
                              ...
                              <td className="event-roles">
                                {event.role?.split(', ').map((role, i) => (
                                  <Badge key={i} className="role-badge">{role.trim()}</Badge>
                                ))}
                              </td>
                              ...
                              <td className="event-actions">
                                {event.htmlLink ? (
                                  ...
                                  >
                                    ...
                                    <span className="d-none d-md-inline">Öffnen</span>
                                  </Button>
                                ) : (
                                  <Button variant="outline-secondary" size="sm" disabled>
                                    <span className="d-none d-md-inline">Nicht verfügbar</span>
                                    <span className="d-md-none">N/A</span>
                                  </Button>
                                )}
                              </td>
                            </tr>
                          ...
                          <div className="event-mobile-content">
                            ...
                            <div className="event-mobile-details">
                              ...
                              {event.start?.dateTime && (
                                <div className="event-mobile-datetime">
                                  <i className="bi bi-calendar-event"></i> {new Date(event.start.dateTime).toLocaleDateString('de-DE')}
                                  <span className="mobile-time">{' '}{new Date(event.start.dateTime).toLocaleTimeString('de-DE', {
                                    hour: '2-digit',
                                    minute: '2-digit'
                                  })}</span>
                                </div>
                            ...
                            <div className="event-mobile-actions">
                              {event.htmlLink ? (
                                ...
                                >
                                  <Calendar3 className="button-icon" />
                                  <span className="button-text">Öffnen</span>
                                </Button>
                              ) : (
                                <Button variant="outline-secondary" size="sm" disabled>
                                  N/A
